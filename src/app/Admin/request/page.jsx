      ...
      if (response.ok) {
        setInterns((prevInterns) =>
          prevInterns.map((intern) =>
            intern.id === id ? { ...intern, progress: 1 } : intern
          )
        );
        console.log("Intern accepted successfully:", data);
      } else {
        console.error(
          "Failed to accept intern:",
      ...
      <main className="flex-1 w-full mx-auto py-4 px-2 sm:px-4 lg:px-6">
        <div className="bg-white rounded-lg shadow-lg overflow-hidden w-full">
          ...
          <div className="w-full p-3 sm:p-4 border-b border-gray-300 flex justify-between items-center bg-gray-700 text-white">
            <h2 className="text-lg sm:text-xl lg:text-2xl font-semibold">Internship Request</h2>
          </div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-200 bg-white text-gray-700 text-xs sm:text-sm">
                  {interns.map((intern, index) => (
                    ...
                    >
                      <td className="px-3 py-2 sm:px-4 sm:py-3 text-center">{index + 1}</td>
              ...
              {interns.map((intern, index) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-600 space-y-1">
                    ...
                    <p><span className="font-medium">University:</span> {intern.university}</p>
                    <p><span className="font-medium">Department:</span> {intern.department}</p>
                    <p><span className="font-medium">Phone:</span> {intern.phone}</p>
                    <p><span className="font-medium">Domain:</span> {intern.domain}</p>
                  </div>
                  ...
                  <div className="flex h-full flex-col bg-white shadow-xl">
                    ...
                    <div className="flex-1 overflow-y-auto p-3 sm:p-4">
                      {selectedIntern && selectedIntern.resume ? (
                        <>
                          <iframe
                            src={selectedIntern.resume}
                            className="w-full h-[80vh] sm:h-full border-none"
                            title="Resume PDF"
                          />
                          <a
                            href={selectedIntern.resume}
                            target="_blank"
                        ...
                        <p className="text-gray-500 text-xs sm:text-sm">No resume available.</p>
                      )}
                    </div>
                  </div>
                </Dialog.Panel>
